...
export class MockTranslationProvider implements TranslationProvider {
  ...
  async translate(input: TranslationInput): Promise<TranslationResult> {
    const delayMs = Number(this.configService.get<string | number>('MOCK_TRANSLATION_DELAY_MS') ?? 300);

    if (input.sourceText.includes('__TIMEOUT__')) {
      const timeoutLimit = Number(this.configService.get<string | number>('TRANSLATION_TIMEOUT_MS') ?? 5000);
