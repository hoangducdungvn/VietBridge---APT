...
FPT Cloud API (`FPT.AI-whisper-large-v3-turbo`) và faster-whisper local (stub, cho demo offline).
...
Sửa `BACKEND` trong [config.py](config.py) — `"fpt"` hoặc `"local"` — hoặc set env `STT_BACKEND=local`. Không cần sửa chỗ nào khác. Backend local cần thêm `pip install faster-whisper`.
...
- `language_hint` là prior; chỉ bị override khi backend detect ngôn ngữ khác với confidence ≥ 0.8 (hiện chỉ backend local báo confidence).
...
- Tham số `language` **bị model bỏ qua**: giọng tiếng Anh bị phiên âm thành phonetic
  ...
  ⚠️ luồng EN→VI cần engine khác (local whisper hoặc API khác). Cần bàn với team.
