...
import { WebAudioCaptureAdapter, type AudioFrame, type DeviceState } from '../audio/captureAdapter';
import { type AudioQuality } from '../audio/resampler';
import { VadEngine, type VadState, type VadEvent } from '../vad/vadEngine';
import { UtteranceManager, type UtteranceCallbacks } from '../utterance/utteranceManager';
import {
  VoiceStreamClient,
...
export class VoicePipeline {
  ...
  private wsClient!: VoiceStreamClient;
  ...
  async start(): Promise<void> {
    ...
    if (sileroLoaded) {
      this.log('✅ AI VAD (Silero) loaded — backend: silero');
    } else {
      this.log('⚠️ Silero unavailable — using energy VAD fallback');
    }
    ...
    const uttCallbacks: UtteranceCallbacks = {
      onUtteranceStart: (info) => {
        this.wsClient.sendUtteranceStart(info);
        this.totalUtterances++;
        this.events.onUtteranceStart?.(info.utterance_id);
        this.log(`utterance.start: ${info.utterance_id} (speaker=${info.speaker_id})`);
      },
      onUtteranceEnd: (info) => {
        this.wsClient.sendUtteranceEnd(info);
        this.events.onUtteranceEnd?.(info.utterance_id, info.reason);
        this.log(`utterance.end: ${info.utterance_id} reason=${info.reason} dur=${info.audio_duration_ms}ms`);
      },
    };
    ...
    this.utteranceManager = new UtteranceManager(
      {
        ...
        speakerState: (this.config.speakerId ? 'assigned' : 'unknown') as SpeakerState,
    ...
    this.wsClient = new VoiceStreamClient(wsConfig, wsEvents);
    ...
    this.capture.onDeviceState((state) => {
      ...
      if (state.type === 'disconnected') {
        ...
        if (this.utteranceManager.isActive()) {
          ...
          this.utteranceManager.forceClose('source_lost', timestampMs, this.wsClient.getCurrentSequence());
        }
        this.events.onError?.('MICROPHONE_DISCONNECTED', 'Audio input device became unavailable');
      } else if (state.type === 'permission_denied') {
        this.log('Microphone permission denied');
        this.events.onError?.('MICROPHONE_PERMISSION_DENIED', 'User denied microphone access');
    ...
    try {
      ...
      this.events.onError?.('MICROPHONE_DISCONNECTED', msg);
      this.running = false;
      return;
    }
  }
  ...
  getMetrics() {
    return {
      ...
      currentSequence: this.wsClient?.getCurrentSequence() ?? 0,
      connectionState: this.wsClient?.getState() ?? 'idle',
  ...
  private handleVadEvent(event: VadEvent): void {
    const currentSeq = this.wsClient.getCurrentSequence();
      ...
      case 'speech_start': {
        ...
        if (preRollFrames.length > 0) {
          ...
          for (const preRollFrame of preRollFrames) {
            ...
            this.sendAudioChunk(pcm, this.chunkCaptureStartMs, (pcm.length / 16000) * 1000, {
              rms_dbfs: -30,
              peak_dbfs: -10,
              estimated_snr_db: 15,
              clipping_ratio: 0,
  ...
  private flushChunkBuffer(_currentTimestampMs: number): void {
    ...
    const totalSamples = this.chunkBuffer.reduce((sum, buf) => sum + buf.length, 0);
    ...
    if (this.utteranceManager.isActive()) {
      this.sendAudioChunk(merged, this.chunkCaptureStartMs, durationMs, avgQuality);
    }
  ...
  private sendAudioChunk(
    ...
    this.wsClient.sendAudioChunk(
      {
        ...
        speech: {
          vad_probability: vadState === 'SPEAKING' || vadState === 'POSSIBLE_END' ? 0.9 : 0.1,
          overlap: false,
    ...
    this.events.onChunkSent?.(this.wsClient.getCurrentSequence());
  }

  private averageQuality(samples: AudioQuality[]): AudioQuality {
    if (samples.length === 0) {
      return { rms_dbfs: -96, peak_dbfs: -96, estimated_snr_db: 0, clipping_ratio: 0 };
    }
    let rms = 0, peak = -Infinity, snr = 0, clip = 0;
